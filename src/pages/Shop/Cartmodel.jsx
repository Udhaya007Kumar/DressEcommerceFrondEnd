...
const CartModel = ({ products, isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed z-[1000] inset-0 bg-gray-900 bg-opacity-80 transition-opacity duration-500 ${
        isOpen ? "opacity-100 visible" : "opacity-5 invisible"
      }`}
    >
      <div
        className={`fixed right-0 top-0 md:w-1/3 w-full bg-white h-full overflow-y-auto transition-transform duration-500 ${
              ...
              products.map((item, index) => (
                ...
                >
                  <div className="flex items-center">
                    <span className="mr-4 px-1 bg-red-400  text-white rounded-full">
                      0{index + 1}
                    </span>
                    <img
                      src={item.image}
